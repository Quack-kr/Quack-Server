...
import static org.quack.QUACKServer.restaurant.domain.QRestaurantArea.restaurantArea;
...
import org.quack.QUACKServer.menu.domain.QMenu;
import org.quack.QUACKServer.menu.domain.QMenuEval;
import org.quack.QUACKServer.menu.enums.MenuEnum;
import org.quack.QUACKServer.restaurant.domain.*;
...
import org.quack.QUACKServer.review.domain.QReviewEvalSummary;
...
public class RestaurantRepositoryImpl implements RestaurantRepositorySupport {
    ...
    private final QRestaurantBreaks restaurantBreaks = QRestaurantBreaks.restaurantBreaks;
    ...
    public RestaurantInfo findRestaurantInfoByRestaurantId(Long restaurantId) {
        Tuple tuple = queryFactory
                ...
                .from(restaurant)
                .leftJoin(restaurantOwnerMetadata).on(restaurantOwnerMetadata.restaurantId.eq(restaurant.restaurantId))
                .where(restaurant.restaurantId.eq(restaurantId))
                .fetchOne();

        List<RestaurantArea> restaurantAreas = queryFactory
                .selectFrom(restaurantArea)
                .where(restaurantArea.restaurant.eq(restaurant))
                .fetch();

        List<RestaurantCategory> restaurantCategories = queryFactory
                .selectFrom(restaurantCategory)
                .where(restaurantCategory.restaurant.eq(restaurant))
                .fetch();

        List<String> restaurantAreaList = new ArrayList<>();

        if (!restaurantAreas.isEmpty()) {
